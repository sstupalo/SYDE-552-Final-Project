...
int main(void)
{
    ...
    int length_pH = sizeof(pH_vals) / sizeof(pH_vals[0]);
    int length_I = sizeof(I_ext_vals) / sizeof(I_ext_vals[0]);

    for (int i = 0; i < length_pH; i++) {
        K_o = initial_K(pH_vals[i]);

        for (int j = 0; j < length_I; j++) {
            run_HH_model(seconds, K_o, pH_vals[i], I_ext_vals[j]);
        }
    }
...
void run_HH_model(int seconds, double K_o, double pH, int current){
	...
	*/
    ...
    snprintf(filename, MAX_FILENAME_LENGTH, "results/%.1f/datafile_%d.dat", pH, current);
